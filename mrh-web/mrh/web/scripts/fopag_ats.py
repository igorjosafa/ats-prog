...
def fopag_ats(usuario:str, senha:str, data_limite:datetime, download_dir = ''):
    if download_dir == '':
        download_dir=os.path.join(os.getcwd(), f'ATS - {data_limite.month}-{data_limite.year}')
    ...
    try:
        ...
        #gera ats com codicao 259 - FOLHA - SERVIDORES CONCURSADOS (Ativos e Cedidos)
        arquivo, fid = m.gerar_ats(condicao = 259, data_limite='{:02}{:02}{}'.format(data_limite.day, data_limite.month, data_limite.year))

        #exporta csv do ats
        m.exportar_relatorio(titulo=None, tipo=7, fname=arquivo+'.csv', fid = fid)

        #gera relatório CSV para comparação
        relatorio = 'ATS_COM_AFASTAMENTOS (oficial)'
        argumentos_p = {'p1': '{:02}/{}'.format(data_limite.month, data_limite.year)}
...
def main():
    parser = argparse.ArgumentParser(
        "fopag-ats",
        description="Script para geração mensal do ATS",
        )
